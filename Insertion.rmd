...
```{r label= "Delete Tables", message=FALSE, eval = F, warning = F, echo=FALSE}
...
```{r label= "tables", eval = T, warning = F, echo=FALSE, message= FALSE}
...
```{r lavel= "dataframe", eval = T, warning = F, echo=FALSE, message=FALSE}
...
```{r label= "Insertion_airports", eval = T, warning = F, echo=FALSE, message=FALSE}
...
```{r label= "Insertion_aircrafts", eval = T, warning = F, echo=FALSE, message=FALSE}
...
```{r label= "Insertion_airlines", eval = T, warning = F, echo=FALSE, message=FALSE}
...
```{r label= "Insertion_wildlife", eval = T, warning = F, echo=FALSE, message=FALSE}
################################################
...
```{r label="Insertion_conditions", eval = T, warning = F, echo=FALSE, message=FALSE}
...
```{r label= "Insertion_flights", message=FALSE, eval = T, warning = F, echo=FALSE}
####################################
...
```{r label= "Insertion_strikes", eval = T, warning = F, echo=FALSE, message=FALSE}
...
```{r label="Display the Inserted value", message=FALSE, eval = T, warning = F, echo=FALSE}
...
```{sql connection=conn, label ="Top Airports with Strikes", message= FALSE, eval = T, warning = F, echo=FALSE}

SELECT airportState AS state, COUNT(*) AS num_incidents
FROM airports
...
```{r label="graph",  message= FALSE, eval = T, warning = F, echo=FALSE}
...
```{r label ="Audit Log Table", eval = T, warning = F, echo=FALSE, message=FALSE}
...
```{sql connection=conn, label ="Drop Procedure", message=FALSE, eval = T, warning = F, echo=FALSE}
DROP PROCEDURE IF EXISTS remove_strike;
```

```{sql connection=conn, label ="SQL Procedure", message=FALSE, eval = T, warning = F, echo=FALSE}

CREATE PROCEDURE remove_strike(IN sid INT)
BEGIN

    -- Log the removal in the audit log
    INSERT INTO audit_log (modification_type, table_name, sid) 
    VALUES ('Removal', 'strikes', sid);
    
    -- Delete the strike from the strikes table
    DELETE FROM strikes WHERE strikes.sid = sid;
...
```{sql connection=conn, label ="Call procedure ", message= FALSE, eval = T, warning = F, echo=FALSE}

CALL remove_strike(10); -- Replace 123 with the actual primary key of the strike
...
```{r label="audit_log output", message= FALSE,  eval = T, warning = F, echo=FALSE}
dbGetQuery(conn, "SELECT * FROM audit_log")
```


Query 1: Average Number of Strikes per Airport
```{sql connection=conn}
SELECT a.airportName, COUNT(s.sid) / COUNT(DISTINCT f.date) AS avg_strikes_per_day
FROM strikes s
...
Query 2: Seasonal Variations in Wildlife Strikes
```{sql connection=conn}
...
Query 3: Impact of Sky Conditions on Wildlife Strikes
```{sql connection=conn}
SELECT c.sky_condition, COUNT(s.sid) AS num_strikes
FROM strikes s
JOIN conditions c ON s.conditions = c.cid
GROUP BY c.sky_condition
...
Query 4: Average Altitude of Strikes
```{sql connection=conn}
SELECT AVG(altitude) AS avg_altitude
FROM strikes;

```
Query 5: Impact of Sky Conditions on Wildlife Strikes
```{sql connection=conn}
SELECT c.sky_condition, COUNT(s.sid) AS num_strikes
FROM strikes s
JOIN conditions c ON s.conditions = c.cid
...
```
Query 6: Airline Strike Frequency
```{sql connection=conn}
SELECT al.name, COUNT(s.sid) AS num_strikes
FROM strikes s
JOIN flights f ON s.fid = f.fid
JOIN airlines al ON f.ailid = al.ailid
GROUP BY al.name
ORDER BY num_strikes DESC
LIMIT 10;
...
Query 7: Impact of Pilot Warnings on Strike Outcomes
```{sql connection=conn}
SELECT s.pilotWarned, COUNT(s.sid) AS num_strikes
FROM strikes s
GROUP BY s.pilotWarned;
```
Query 8: Yearly Trend in Wildlife Strikes
```{sql connection=conn}
SELECT EXTRACT(YEAR FROM f.date) AS year, COUNT(s.sid) AS num_strikes
FROM strikes s
JOIN flights f ON s.fid = f.fid
...
```
Query 9: Top Aircraft Models Involved in Strikes
```{sql connection=conn}
SELECT ac.model, COUNT(s.sid) AS num_strikes
FROM strikes s
JOIN flights f ON s.fid = f.fid
JOIN aircrafts ac ON f.airid = ac.airid
GROUP BY ac.model
ORDER BY num_strikes DESC
LIMIT 10;
```
Query 10: Strike Frequency by Airport State
```{sql connection=conn}
SELECT a.airportState, COUNT(s.sid) AS num_strikes
FROM strikes s
JOIN flights f ON s.fid = f.fid
JOIN airports a ON f.originAirport = a.aid
